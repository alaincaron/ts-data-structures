...
describe('ArraySet', () => {
  describe('constructor', () => {
    ...
    it('should have specified capacity', () => {
      const set = ArraySet.create({ capacity: 2 });
    ...
    it('should have the same elements as the array argument', () => {
      const arr = [1, 2];
      const set = ArraySet.create({ capacity: 2, initial: arr });
      expect(set.capacity()).equal(2);
    ...
    it('should be identical to the ArraySet argument', () => {
      const arr = [1, 2];
      const set1 = ArraySet.create({ capacity: 3, initial: arr });
      expect(set1.capacity()).equal(3);
      const set2 = ArraySet.create({ initial: set1 });
    ...
    it('should be identical to the Collection argument', () => {
      const arr = [1, 2];
      const set1 = ArraySet.create({ initial: arr });
      const set2 = ArraySet.create({ initial: set1 });
    ...
    it('should use the function provided in the ArrayGenerator', () => {
      ...
      const set = ArraySet.create({ initial: { length: arr.length, seed: i => i + 1 } });
      expect(set.toArray()).to.deep.equal(arr);
    });

    it('should use the iterator provided in the ArrayGenerator', () => {
      const set = ArraySet.create({ initial: { length: 10, seed: Generators.range() } });
    ...
    it('should use the iterable provided in the ArrayGenerator', () => {
      ...
      const set = ArraySet.create({ initial: { length: 10, seed: arr } });
      expect(set.size()).equal(2);
      expect(set.toArray()).to.deep.equal(arr);
    });

    it('should throw if number of initial elements exceed capacity', () => {
      expect(() => ArraySet.create({ capacity: 0, initial: { length: 10, seed: i => i + 1 } })).to.throw(
        OverflowException
      );
    });
  });
  ...
  describe('add', () => {
    ...
    it('should throw if full and element not present', () => {
      const set = ArraySet.create({ capacity: 1 });
      expect(set.add(1)).to.be.true;
      expect(set.isFull()).to.be.true;
      expect(set.add(1)).to.be.false;
  ...
  describe('clear', () => {
    it('should clear the content', () => {
      const set = ArraySet.create({ capacity: 3, initial: { length: 2, seed: (i: number) => i } });
  ...
  describe('removeItem', () => {
    ...
    it('should return false if item is missing', () => {
      const arr = [1, 2, 3];
      const set = ArraySet.create({ initial: arr });
  ...
  describe('offerFully', () => {
    it('should refuse all the items if not enough capacity remaining', () => {
      ...
      expect(set.isEmpty()).to.be.true;
      expect(set.offerFully(ArraySet.create({ initial: data }))).equal(0);
      expect(set.isEmpty()).to.be.true;
    });
    it('should accept all items if enough capacity remaining', () => {
      const set = ArraySet.create({ capacity: 6 });
      const data = [1, 2, 3, 1, 3, 2];
      expect(set.offerFully(data)).equal(3);
      expect(set.size()).equal(3);
      expect(set.offerFully(ArraySet.create({ initial: [1, 2, 3, 4, 5, 6] }))).equal(3);
  ...
  describe('offerPartially', () => {
    it('should accept elements up to the remaining capacity', () => {
      ...
      expect(set.toArray()).to.deep.equal([1, 2]);
      set.clear();
      expect(set.offerPartially(ArraySet.create({ initial: data }))).equal(2);
      expect(set.toArray()).to.deep.equal([1, 2]);
    });
    it('should accept all items if enough capacity remaining', () => {
      const set = ArraySet.create({ capacity: 6 });
      ...
      expect(set.offerPartially(ArraySet.create({ initial: data }))).equal(0);
      expect(set.size()).equal(5);
    });
    it('should accept all the items if there is enough capacity remaining for distinct elements', () => {
      const set = ArraySet.create({ capacity: 2 });
  ...
  describe('toJSON', () => {
    it('should return the JSON string', () => {
      const set = ArraySet.create({ initial: [1, 2, 3] });
