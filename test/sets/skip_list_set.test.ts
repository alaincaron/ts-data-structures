...
import { OverflowException, shuffle, SkipListSet } from '../../src';

describe('SkipListSet', () => {
  describe('constructor', () => {
    it('should have infinite capacity as per default ctor', () => {
      const set = new SkipListSet();
    ...
    it('should use the specified capacity as per options', () => {
      const set = SkipListSet.create({ capacity: 2 });
      expect(set.capacity()).equal(2);
      expect(set.isEmpty()).to.be.true;
    });
    ...
    it('should have the same elements as the array argument', () => {
      const arr = [2, 1, 3];
      const set = SkipListSet.create({ capacity: arr.length, initial: arr });
    ...
    it('should be identical to the SkipListSet argument', () => {
      const arr = [3, 1, 2];
      const set1 = SkipListSet.create({ capacity: arr.length, initial: arr });
      expect(set1.capacity()).equal(arr.length);
      const set2 = SkipListSet.create({ initial: set1 });
    ...
    it('should be identical to the Collection argument', () => {
      const arr = [2, 1, 3];
      const set1 = SkipListSet.create({ initial: arr });
      const set2 = SkipListSet.create({ initial: set1 });
    ...
    it('should use the function provided in the ArrayGenerator', () => {
      ...
      const set = SkipListSet.create({ initial: { length: arr.length, seed: i => arr[i] } });
      expect(set.toArray()).to.deep.equal(arr.sort());
    });

    it('should use the iterator provided in the ArrayGenerator', () => {
      const set = SkipListSet.create({ initial: { length: 10, seed: Generators.range() } });
    ...
    it('should use the iterable provided in the ArrayGenerator', () => {
      ...
      const set = SkipListSet.create({ initial: { length: arr.length, seed: arr } });
      expect(set.size()).equal(arr.length);
      expect(set.toArray()).to.deep.equal(arr.sort());
    });

    it('should throw if number of initial elements exceed capacity', () => {
      expect(() => SkipListSet.create({ capacity: 0, initial: { length: 10, seed: i => i + 1 } })).to.throw(
        OverflowException
      );
    });
  });
  ...
  describe('clear', () => {
    it('should clear the content', () => {
      const set = SkipListSet.create({ capacity: 3, initial: { length: 2, seed: (i: number) => i } });
  ...
  describe('contains', () => {
    ...
    it('should return false if absent', () => {
      const set = SkipListSet.create({ initial: { length: 10, seed: (i: number) => i } });
  ...
  describe('removeItem', () => {
    ...
    it('should return false if item is missing', () => {
      const arr = [1, 2, 3];
      const set = SkipListSet.create({ initial: arr });
  ...
  describe('offerFully', () => {
    it('should refuse all the items if not enough capacity remaining', () => {
      ...
      expect(set.isEmpty()).to.be.true;
      expect(set.offerFully(SkipListSet.create({ initial: data }))).equal(0);
      expect(set.isEmpty()).to.be.true;
    });
    it('should accept all items if enough capacity remaining', () => {
      const set = new SkipListSet(6);
      const data = [1, 2, 3, 1, 3, 2];
      expect(set.offerFully(data)).equal(3);
      expect(set.size()).equal(3);
      expect(set.offerFully(SkipListSet.create({ initial: [1, 2, 3, 4, 5, 6] }))).equal(3);
  ...
  describe('offerPartially', () => {
    ...
    it('should accept all items if enough capacity remaining', () => {
      ...
      expect(set.offerPartially(SkipListSet.create({ initial: data }))).equal(0);
      expect(set.size()).equal(5);
    });
    it('should accept all the items if there is enough capacity remaining for distinct elements', () => {
      const set = new SkipListSet(2);
  ...
  describe('forEach', () => {
    it('should execute for each item', () => {
      const data = [1, 2, 3];
      const set = SkipListSet.create({ initial: data });
      const x: number[] = [];
  ...
  describe('fold', () => {
    ...
    it('should return initial value if empty', () => {
      const set = new SkipListSet<number>();
      const sum = set.fold((a, b) => a + b, 1);
      expect(sum).equal(1);
    });
  });
  ...
  describe('toJson', () => {
    it('should return the JSON string', () => {
      const set = SkipListSet.create({ initial: [4, 2, 1, 3, 2, 3, 4] });
