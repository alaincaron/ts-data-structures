...
describe('window', () => {
  ...
  it('should compute a moving average with window of 2', () => {
    const actual = iterator([1, 2, 3, 4, 5]).transform(windowIteratorMapper(new MovingAverageCollector(), 2)).collect();
  ...
  it('should compute a moving average with window of 3', () => {
    const actual = iterator([1, 2, 3, 4, 5]).transform(windowIteratorMapper(new MovingAverageCollector(), 3)).collect();
