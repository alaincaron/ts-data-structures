...
describe('SkipListMultiSet', () => {
  describe('constructor', () => {
    ...
    it('should have the same elements as the array argument', () => {
      const arr = [1, 2, 1];
      const set = SkipListMultiSet.create({ capacity: 3, initial: arr });
    ...
    it('should be identical to the SkipListMultiSet argument', () => {
      const arr = [1, 2, 1];
      const set1 = SkipListMultiSet.create({ capacity: 3, initial: arr });
      expect(set1.capacity()).equal(3);
      const set2 = SkipListMultiSet.create({ initial: set1 });
    ...
    it('should be identical to the Collection argument', () => {
      const arr = [1, 2];
      const set1 = SkipListMultiSet.create({ initial: arr });
      const set2 = SkipListMultiSet.create({ initial: set1 });
    ...
    it('should use the function provided in the ArrayGenerator', () => {
      ...
      const set = SkipListMultiSet.create({ initial: { length: arr.length, seed: i => i + 1 } });
      expect(set.toArray()).to.deep.equal(arr);
    });

    it('should use the iterator provided in the ArrayGenerator', () => {
      const set = SkipListMultiSet.create({ initial: { length: 10, seed: Generators.range() } });
    ...
    it('should use the iterable provided in the ArrayGenerator', () => {
      ...
      const set = SkipListMultiSet.create({ initial: { length: 10, seed: arr } });
      expect(set.size()).equal(2);
      expect(set.toArray()).to.deep.equal(arr);
    });

    it('should throw if number of initial elements exceed capacity', () => {
      expect(() => SkipListMultiSet.create({ capacity: 0, initial: { length: 10, seed: i => i + 1 } })).to.throw(
        OverflowException
      );
    });
  });
  ...
  describe('clone', () => {
    it('should create a deep equal copy', () => {
      const a = new SkipListMultiSet();
      const b = a.clone();
      expect(b instanceof SkipListMultiSet).to.be.true;
  ...
  describe('clear', () => {
    it('should clear the content', () => {
      const set = SkipListMultiSet.create({ capacity: 3, initial: { length: 2, seed: (i: number) => i } });
  ...
  describe('removeItem', () => {
    ...
    it('should return false if item is missing', () => {
      const arr = [1, 2, 3];
      const set = SkipListMultiSet.create({ initial: arr });
  ...
  describe('offerFully', () => {
    it('should refuse all the items if not enough capacity remaining', () => {
      ...
      expect(set.isEmpty()).to.be.true;
      expect(set.offerFully(SkipListMultiSet.create({ initial: data }))).equal(0);
      expect(set.isEmpty()).to.be.true;
    });
    it('should accept all items if enough capacity remaining', () => {
      ...
      expect(set.offerFully(SkipListMultiSet.create({ initial: [1, 2, 3, 4, 5, 6] }))).equal(0);
      expect(set.size()).equal(6);
    });
  });
  ...
  describe('offerPartially', () => {
    it('should accept elements up to the remaining capacity', () => {
      ...
      set.clear();
      expect(set.offerPartially(SkipListMultiSet.create({ initial: data }))).equal(2);
      expect(set.toArray()).to.deep.equal([1, 2]);
    });
    it('should accept all items if enough capacity remaining', () => {
      const set = SkipListMultiSet.create({ capacity: 6 });
      ...
      expect(set.size()).equal(6);
      expect(set.offerPartially(SkipListMultiSet.create({ initial: data }))).equal(0);
      expect(set.size()).equal(6);
    });
  ...
  describe('toJSON', () => {
    it('should return the JSON string', () => {
      const set = SkipListMultiSet.create({ initial: [1, 2, 3, 4, { x: true }, 'alain'] });
      const set2 = SkipListMultiSet.create({ initial: JSON.parse(set.toJSON()) });
  ...
  describe('setCount', () => {
    it('should remove items when set to 0', () => {
      const ms = SkipListMultiSet.create({ initial: ['foo', 'bar', 'foo'] });
    ...
    it('should increase the size if new count is greater', () => {
      const ms = SkipListMultiSet.create({ initial: ['foo'] });
    ...
    it('should decrease the size if new count is smaller', () => {
      const ms = SkipListMultiSet.create({ initial: ['foo', 'foo'] });
