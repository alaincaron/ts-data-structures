...
import { OpenHashMultiSet, OverflowException } from '../../src';

describe('OpenHashMultiSet', () => {
  describe('constructor', () => {
    it('should have infinite capacity as per default ctor', () => {
      const set = new OpenHashMultiSet();
    ...
    it('should have the same elements as the array argument', () => {
      const arr = [1, 2, 1];
      const set = OpenHashMultiSet.create({ capacity: 3, initial: arr });
    ...
    it('should be identical to the OpenHashMultiSet argument', () => {
      const arr = [1, 2, 1];
      const set1 = OpenHashMultiSet.create({ capacity: 3, initial: arr });
      expect(set1.capacity()).equal(3);
      const set2 = OpenHashMultiSet.create({ initial: set1 });
    ...
    it('should be identical to the Collection argument', () => {
      const arr = [1, 2];
      const set1 = OpenHashMultiSet.create({ initial: arr });
      const set2 = OpenHashMultiSet.create({ initial: set1 });
    ...
    it('should use the function provided in the ArrayGenerator', () => {
      ...
      const set = OpenHashMultiSet.create({ initial: { length: arr.length, seed: i => i + 1 } });
      expect(set.toArray().sort()).to.deep.equal(arr);
    });

    it('should use the iterator provided in the ArrayGenerator', () => {
      const set = OpenHashMultiSet.create({ initial: { length: 10, seed: Generators.range() } });
    ...
    it('should use the iterable provided in the ArrayGenerator', () => {
      ...
      const set = OpenHashMultiSet.create({ initial: { length: 10, seed: arr } });
      expect(set.size()).equal(2);
      expect(set.toArray().sort()).to.deep.equal(arr);
    });

    it('should throw if number of initial elements exceed capacity', () => {
      expect(() => OpenHashMultiSet.create({ capacity: 0, initial: { length: 10, seed: i => i + 1 } })).to.throw(
        OverflowException
      );
    });
  });
  ...
  describe('clone', () => {
    it('should create a deep equal copy', () => {
      const a = new OpenHashMultiSet();
      const b = a.clone();
      expect(b instanceof OpenHashMultiSet).to.be.true;
  ...
  describe('clear', () => {
    it('should clear the content', () => {
      const set = OpenHashMultiSet.create({ capacity: 3, initial: { length: 2, seed: (i: number) => i } });
  ...
  describe('contains', () => {
    ...
    it('should return false if absent', () => {
      const set = OpenHashMultiSet.create({ initial: { length: 10, seed: (i: number) => i } });
  ...
  describe('removeItem', () => {
    ...
    it('should return false if item is missing', () => {
      const arr = [1, 2, 3];
      const set = OpenHashMultiSet.create({ initial: arr });
  ...
  describe('offerFully', () => {
    it('should refuse all the items if not enough capacity remaining', () => {
      ...
      expect(set.isEmpty()).to.be.true;
      expect(set.offerFully(OpenHashMultiSet.create({ initial: data }))).equal(0);
      expect(set.isEmpty()).to.be.true;
    });
    it('should accept all items if enough capacity remaining', () => {
      ...
      expect(set.offerFully(OpenHashMultiSet.create({ initial: [1, 2, 3, 4, 5, 6] }))).equal(0);
      expect(set.size()).equal(6);
    });
  });
  ...
  describe('offerPartially', () => {
    ...
    it('should accept all items if enough capacity remaining', () => {
      ...
      expect(set.size()).equal(6);
      expect(set.offerPartially(OpenHashMultiSet.create({ initial: data }))).equal(0);
      expect(set.size()).equal(6);
    });
  ...
  describe('forEach', () => {
    it('should execute for each item', () => {
      const data = [1, 2, 3];
      const set = OpenHashMultiSet.create({ initial: data });
      const x: number[] = [];
  ...
  describe('fold', () => {
    ...
    it('should return initial value if empty', () => {
      const set = new OpenHashMultiSet<number>();
      const sum = set.fold((a, b) => a + b, 1);
      expect(sum).equal(1);
    });
  });
  ...
  describe('toJson', () => {
    it('should return the JSON string', () => {
      const set = OpenHashMultiSet.create({ initial: [1, 2, 3, 4, { x: true }, 'alain'] });
      const set2 = OpenHashMultiSet.create({ initial: JSON.parse(set.toJson()) });
