...
describe('OpenHashMapCollector', () => {
  ...
  it('should add items to the built collection using the default collision handler', () => {
    ...
    const collector = openHashMapCollector({ map });
    collector.collect(['foo', 6]);
    collector.collect(['foo', 3]);
    const result = collector.result;
  ...
  it('should throw on collision', () => {
    const map = new OpenHashMap();
    const collector = openHashMapCollector({ map, collisionHandler: Functions.CollisionHandlers.reject });
    collector.collect(['foo', 3]);
  ...
  it('should keep previous on collision', () => {
    const map = new OpenHashMap();
    const collector = openHashMapCollector({ map, collisionHandler: Functions.CollisionHandlers.ignore });
    collector.collect(['foo', 3]);
    collector.collect(['foo', 6]);
    const result = collector.result;
    expect(result).to.equal(map);
  ...
  it('should overwrite previous value on collision', () => {
    const map = new OpenHashMap();
    const collector = openHashMapCollector({ map, collisionHandler: Functions.CollisionHandlers.overwrite });
    collector.collect(['foo', 3]);
    collector.collect(['foo', 6]);
    const result = collector.result;
    expect(result).to.equal(map);
