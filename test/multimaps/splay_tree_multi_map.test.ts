...
import { MutableCollection, MutableMapEntry, OverflowException, SplayTreeMultiMap } from '../../src';

describe('SplayTreeMultiMap', () => {
  describe('constructor', () => {
    ...
    it('should have specified capacity', () => {
      const map = SplayTreeMultiMap.create({ capacity: 2 });
    ...
    it('should initialize with the provided Map', () => {
      const map = SplayTreeMultiMap.create({ initial: new Map().set('a', 1).set('b', 2) });
    ...
    it('should initialize with the provided IMap', () => {
      const map1 = new SplayTreeMultiMap();
      map1.put('a', 1);
      map1.put('b', 2);
      const map = SplayTreeMultiMap.create({ initial: map1 });
      expect(map.size()).equal(2);
      expect(map.getValues('a')?.equals(list(1))).to.be.true;
      expect(map.getValues('b')?.equals(list(2))).to.be.true;
    });
    ...
    it('should initialize with the provided Iterable', () => {
      const map = SplayTreeMultiMap.create({
        initial: [
          ['a', 1],
          ['b', 2],
          ['a', 3],
  ...
  describe('put/getValues', () => {
    ...
    it('should throw if adding a new element and map is full', () => {
      const map = SplayTreeMultiMap.create({ capacity: 1 });
  ...
  describe('clear', () => {
    it('should clear the content', () => {
      const map = SplayTreeMultiMap.create({ capacity: 3 });
      map.put('a', 1);
      map.put('b', 2);
  ...
  describe('filterKeys', () => {
    it('should remove keys not matching predicate', () => {
      const map = new SplayTreeMultiMap<string, number>();
      map.put('foo', 1);
      map.put('bar', 2);
      map.put('foobar', 3);
