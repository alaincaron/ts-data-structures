...
import { MultiMapInitializer, MultiMapLike, MultiMapOptions } from './types';
...
export abstract class AbstractMultiMap<K, V> implements MultiMap<K, V>, OptionsBuilder {
  ...
  *partitions(): IterableIterator<[K, Collection<V>]> {
  ...
  equals(other: any) {
    ...
    if (typeof other.getValues !== 'function') return false;
    for (const [k, values] of this.rawIterator()) {
...
export const BoundedMultiMap = CapacityMixin(AbstractMultiMap);
