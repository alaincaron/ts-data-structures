...
export function nextPrime(desiredCapacity: number): number {
  ...
  let i = bsearch(primeCapacities, desiredCapacity);
  if (i < 0) i = ~i;
  return primeCapacities[i];
}

export function nextPowerOfTwo(capacity: number): number {
  if (capacity > MAX_ARRAY_SIZE) throw new Error('Array too large');
  --capacity;
  capacity |= capacity >> 1;
  capacity |= capacity >> 2;
  capacity |= capacity >> 4;
