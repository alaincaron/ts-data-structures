...
export function toShellSorted<T>(arr: T[], left: number, right: number, random: Comparator<T> | undefined): T[];
...
export function toShellSorted<T>(
  ...
  return shellSort0(result, 0, result.length, compare);
}
...
export function shellSort<T>(
  ...
  const { left, right, f: compare } = parseArgs(arr, arg2, arg3, arg4, Comparators.natural);
  return shellSort0(arr, left, right, compare);
}

function shellSort0<T>(arr: T[], left: number, right: number, compare: Comparator<T>): T[] {
